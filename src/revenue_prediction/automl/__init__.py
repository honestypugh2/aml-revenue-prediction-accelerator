...
This module builds and (optionally) submits an AutoML *regression* job to
predict ``actual_month_end_net_revenue``. It uses the Azure ML Python SDK v2
exclusively (no deprecated SDK v1 APIs).

Imports of ``azure.ai.ml`` are deferred so the module can be imported and unit
...
from ..config.models import AutoMLConfig, AzureMLConfig
...
def submit_automl_job(ml_client: MLClient, spec: AutoMLJobSpec) -> Any:  # pragma: no cover
    ...
    This function is deliberately excluded from offline tests. It returns the
